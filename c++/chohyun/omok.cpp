...
int count_h(int data[BSIZE][BSIZE], int row, int col) {
   ...
   if (data[i][j] == ROUND) {
      while (i<BSIZE-1 && data[i][j - 1] == ROUND) {
         count++;
         j--;
      }
      while (data[i][j - 1] < data[BSIZE][BSIZE] && data[i][j + 1] == ROUND) {
         count++;
         j++;
      }
      ...
      while (data[i][j - 1] < data[BSIZE][BSIZE] && data[i][j - 1] == SQUARE) {
         count++;
         j--;
      }
      while (data[i][j - 1] < data[BSIZE][BSIZE] && data[i][j + 1] == SQUARE) {
